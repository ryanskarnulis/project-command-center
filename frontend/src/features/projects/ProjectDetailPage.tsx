...
export function ProjectDetailPage() {
  ...
  // Below 720px the route is the M06f brief (no task list); desktop keeps its own tree.
  ...
  // rest of the project page usable. On mobile the list is data only — it feeds
  // the header counts and never renders.
  useEffect(() => {
    let active = true
    listTasks(id)
      .then((data) => {
  ...
  // Each field anchors to its own server value, so one field's save landing
  // doesn't discard an in-progress edit to the other.
  ...
  if (mobile) {
    return (
      <MobileProjectOverview
        ...
        tasksError={tasksLoading ? null : tasksError}
        nameDraft={nameDraft}
        descriptionDraft={descriptionDraft}
        onNameChange={nameField.set}
        onDescriptionChange={descriptionField.set}
        saveName={saveName}
        ...
        onDelete={() => fireAndForget(handleDeleteProject())}
      />
    )
  }
    ...
    <main className="task-detail">
      ...
      <section className="task-detail-panel task-description-panel">
        ...
        <textarea
          ...
          onChange={(e) =>
            descriptionField.set(e.target.value)
          }
