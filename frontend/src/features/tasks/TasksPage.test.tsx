...
describe('TasksPage', () => {
  ...
  describe('mobile handoff', () => {
    ...
    beforeEach(() => {
      ...
      mockListProjects.mockResolvedValue([{ ...baseProject, id: 1, name: 'Homelab Migration' }])
      mockListCompletedTasks.mockResolvedValue([])
    ...
    it('ignores board mode, names the project and opens a project-prefilled create dialog', async () => {
      const user = userEvent.setup()
      renderProject(['/tasks?view=board'])
    ...
    it('keeps multi-priority edits pending, discards dismissal, applies and removes visible URL filters', async () => {
      const user = userEvent.setup()
      mockListTasks.mockResolvedValue([baseTask, { ...baseTask, id: 2, title: 'Certificate', priority: 'high', due_date: '2020-01-01' }, { ...baseTask, id: 3, title: 'Urgent work', priority: 'urgent' }])
      const { router } = renderProject()
      await screen.findByText('Fix the VPN')
      await user.click(screen.getByRole('button', { name: 'Filter tasks' }))
      let sheet = within(screen.getByRole('dialog', { name: 'Filters' }))
      await user.click(sheet.getByRole('button', { name: 'High' }))
      await user.click(sheet.getByRole('button', { name: 'Urgent' }))
      expect(sheet.getByRole('button', { name: 'Show 2 tasks' })).toBeEnabled()
      expect(router.state.location.search).not.toContain('priority')
      expect(screen.getByText('Fix the VPN')).toBeInTheDocument()
      await user.click(sheet.getByRole('button', { name: 'Close filters' }))
      await user.click(screen.getByRole('button', { name: 'Filter tasks' }))
      sheet = within(screen.getByRole('dialog', { name: 'Filters' }))
      expect(sheet.getByRole('button', { name: 'High' })).toHaveAttribute('aria-pressed', 'false')
      await user.click(sheet.getByRole('button', { name: 'High' }))
      await user.click(sheet.getByRole('button', { name: 'Urgent' }))
      await user.click(sheet.getByRole('switch', { name: 'Overdue' }))
      await user.click(sheet.getByRole('button', { name: 'Show 1 task' }))
      expect(screen.queryByText('Fix the VPN')).toBeNull()
      expect(screen.getByText('Certificate')).toBeInTheDocument()
      expect(new URLSearchParams(router.state.location.search).get('priority')).toBe('urgent,high')
      expect(screen.getByRole('button', { name: 'Remove Priority: High+' })).toBeInTheDocument()
    ...
    it('discloses nested tasks per parent and promotes a matching child when its parent is filtered out', async () => {
      const user = userEvent.setup()
      mockListTasks.mockResolvedValue([{ ...baseTask, has_subtasks: true }, { ...baseTask, id: 2, parent_task_id: 1, title: 'VPN child', priority: 'urgent' }])
      const { router } = renderProject()
      await screen.findByText('Fix the VPN')
      expect(screen.queryByText('VPN child')).toBeNull()
      expect(screen.getByText('1 subtask')).toBeInTheDocument()
      expect(screen.getByRole('button', { name: 'Mark Fix the VPN done' })).toBeDisabled()
      await user.click(screen.getByRole('button', { name: 'Expand subtasks of Fix the VPN' }))
      expect(screen.getByText('VPN child')).toBeInTheDocument()
      await act(() => router.navigate('/projects/1/tasks?priority=urgent'))
      expect(screen.queryByText('Fix the VPN')).toBeNull()
      expect(screen.getByText('VPN child')).toBeInTheDocument()
    })

    it('refreshes completed parents and counts after reopening their last completed child', async () => {
      const user = userEvent.setup()
      const parent: Task = { ...baseTask, title: 'Completed parent', workflow_status: 'done', has_subtasks: true }
      const child: Task = { ...baseTask, id: 2, parent_task_id: 1, title: 'Completed child', workflow_status: 'done' }
      mockListTasks.mockResolvedValue([])
      mockListCompletedTasks.mockResolvedValue([parent, child])
      mockReopenTask.mockResolvedValue({ ...child, workflow_status: 'open' })
      renderProject(['/tasks?status=done'])
      await screen.findByText('Completed parent')
      await user.click(screen.getByRole('button', { name: 'Expand subtasks of Completed parent' }))
      mockListCompletedTasks.mockResolvedValue([])
      mockListTasks.mockResolvedValue([{ ...parent, workflow_status: 'open' }, { ...child, workflow_status: 'open' }])
      await user.click(screen.getByRole('button', { name: 'Reopen Completed child' }))
      await waitFor(() => expect(screen.queryByText('Completed parent')).toBeNull())
      expect(screen.getByRole('button', { name: 'Hide done · 0' })).toBeInTheDocument()
    })

    it('shows the completed archive for a Done URL and preserves the recurrence-safe completion path', async () => {
      const user = userEvent.setup()
      mockListCompletedTasks.mockResolvedValue([{ ...baseTask, id: 2, title: 'Archived task', workflow_status: 'done' }])
      const { router } = renderProject(['/tasks?status=done&view=board'])
      expect(await screen.findByText('Archived task')).toBeInTheDocument()
      expect(screen.queryByText('Fix the VPN')).toBeNull()
      expect(screen.getByRole('button', { name: 'Remove Status: Done' })).toBeInTheDocument()
      await act(() => router.navigate('/projects/1/tasks'))
      await user.click(screen.getByRole('button', { name: 'Mark Fix the VPN done' }))
      await waitFor(() => expect(mockMarkTaskDone).toHaveBeenCalledWith(1))
